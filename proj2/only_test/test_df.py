...
from proj2.data_provider.data_loader_pd import Dataset_neural_pd
...
c_out = 2
# 使用批训练方式
dataset = Dataset_neural_pd(c_out, enc_in)
...
for epoch in range(10):
    ...
    if loss.item() < best_loss:
        #print('save model')
        best_loss = loss.item()
        torch.save(net.state_dict(), 'test_record/no_seq/best.pth')

# 使用训练好的模型进行预测
net.load_state_dict(torch.load('test_record/no_seq/best.pth'))
net.eval()
dataset = Dataset_neural_pd(c_out, enc_in)
...
# 绘图展示预测的和真实数据之间的差异
visual_all(all_true, all_pred, 'test_record/no_seq/neural_pd_lstm.png', c_out)